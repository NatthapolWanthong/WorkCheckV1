...
(function ($) {
  ...
  const appState = {
    ...
    fp: null,
    lastPickedDate: null,
    originalData: {}, // snapshot from server: { [user_id]: { ... } }
    editedData: {},   // local draft changes: { [user_id]: { field: val } }
  ...
  /**
   * updates: partial state to merge into appState
   * opts: { refreshTable: boolean, redrawDatepicker: boolean, skipUIUpdate: boolean }
   */
  ...
  // -------------------- UI updater (top-level, must exist before usage) --------------------
  function updateUIButtons() {
    // show/hide controls based on appState and permissions
    if (appState.editMode) {
      ...
      // show state column only if multiEditor active
      try {
  ...
  // -------------------- UI helpers for edit mode / table --------------------
  ...
  // central UI update helper (consolidates commonly used UI updates)
  function updateTableUi(opts = {}) {
    // opts: { initFlatpickr: bool, ensureClearButtons: bool, applyDrafts: bool, applySelections: bool, updateEnabledStates: bool }
    try {
  ...
  // -------------------- VALIDATION / RECORD COLLECTION (separated) --------------------
  ...
  $(function () {
    ...
    window.addEventListener('beforeunload', function (e) {
      if (appState.editMode) {
        const msg = 'ยังไม่ได้บันทึกการเปลี่ยนแปลง — แน่ใจหรือว่าต้องการออก?';
        e.returnValue = msg;
        return msg;
      }
    });

    // nav capture when editing
    function navCapture(ev) {
      ...
      if (confirm('ยังไม่ได้บันทึกการเปลี่ยนแปลง ต้องการออกโดยไม่บันทึกหรือไม่?')) {
        document.removeEventListener('click', navCapture, true);
        setTimeout(() => el.click(), 10);
        setTimeout(() => document.addEventListener('click', navCapture, true), 200);
      }
    }
    document.addEventListener('click', navCapture, true);

    // -------------------- DELEGATED INPUT HANDLERS --------------------
    $(document).on('click', '.btn-clear-ot', function () {
      const $input = $(this).siblings('input');
      if ($input[0] && $input[0]._flatpickr) $input[0]._flatpickr.clear();
      $input.val('').trigger('change');
    });

    $(document).on('click', '.btn-clear-ot-result', function (e) {
      e.preventDefault();
      try {
    ...
    $(document).on('input change', '#attendanceTable input, #attendanceTable textarea', function () {
      const $self = $(this);
    ...
    $(document).on('change', '#attendanceTable .present-checkbox', function () {
      const uid = String($(this).data('user'));
      if (!uid) return;
      if (!appState.editedData[uid]) appState.editedData[uid] = {};
    ...
    // -------------------- LOAD PERMISSIONS, DATES, DEPARTMENTS --------------------
    $.getJSON(`${API_BASE}/get_departments.php`)
      .done(function (data) {
        ...
        console.debug('[Attendance] loadUserPermissions ->', appState.userPermissions);
      })
      .fail(function () {
    ...
    function enterEditMode() {
      // set editMode and refresh table so formatters re-render inputs enabled
      updateAppState({ editMode: true }, { refreshTable: true });
      // after refresh, update UI components (flatpickr, clear buttons, drafts, states)
      // (onPostBody will also run; keep this to ensure immediate client-side init)
      updateTableUi();
    }
    ...
    $btnSave.on('click', function () {
      // validate & collect using separated functions
    ...
    $btnTogglePresent.on('click', function () {
      if (!appState.editMode) return;
    ...
    $btnMultiEditor.on('click', function () {
      if (!appState.editMode) return;
    ...
    window._debug_permissions = function () {
      console.debug('[Attendance] mock user:', user);
      console.debug('[Attendance] userPermissions:', appState.userPermissions);
      console.debug('[Attendance] departmentsList:', appState.departmentsList);
      console.debug('[Attendance] selectedRows:', Array.from(appState.selectedRows));
    };

  }); // end $(function)

  // -------------------- CLEANUP (exposed for debug) --------------------
  try {
